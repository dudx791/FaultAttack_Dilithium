...
static void keccak_absorb1(uint64_t *s, uint32_t r, const uint8_t *m,
                          size_t mlen, uint8_t p,uint8_t *pp) {
    size_t i;
    uint8_t t[200];
...
void shake256_absorb1(shake256ctx *state, const uint8_t *input, size_t inlen,uint8_t *p) {
    state->ctx = malloc(PQC_SHAKECTX_BYTES);
    if (state->ctx == NULL) {
        exit(111);
    }
    keccak_absorb1(state->ctx, SHAKE256_RATE, input, inlen, 0x1F,p);
}
/*************************************************
...
void shake2561(uint8_t *output, size_t outlen,
              const uint8_t *input, size_t inlen,uint8_t *p) {
    size_t nblocks = outlen / SHAKE256_RATE;
    uint8_t t[SHAKE256_RATE];
    shake256ctx s;
    shake256_absorb1(&s, input, inlen,p);
    shake256_squeezeblocks(output, nblocks, &s);
